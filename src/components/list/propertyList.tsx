...
export default function PropertyList(props) {
    ...
    function filterList(event) {
        ...
        listItemsFiltered = properties.filter((element, index) => {
            ...
            <div className="bg-gray-100 border-gray-200 px-4 py-5 sm:px-6">
                ...
                <div className="flex w-full">
                    <div className="w-full">
                        <h3 className="text-lg leading-6 font-medium text-gray-900">Lista de propriedade</h3>
                ...
                <form className="mt-5 flex" onSubmit={filterList}>
                    <div className="w-full self-end">
                        <label htmlFor="property-search-input" className={subtitle}>
                            Pesquisa
                        </label>
                        <input
                            onChange={(event) => {
            ...
            <div className="grid grid-cols-1 gap-4 p-4 bg-white">
                {listItems.map((element, index) => (
                    <button key={index.toString() + element.codigoBarras}
                        ...
                        <div><span className={contentClassName}>{element.municipio}</span></div>
                    </button>
                ))}
            </div>
