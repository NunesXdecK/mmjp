...
const handleAddImmobilePoint = async (immobileId, pointId) => {
    ...
    let data: any = {
        immobileId: point.posnHeightQuality,
        pointId: point.posnHeightQuality,
    }
    let id = point?.id ?? 0
    try {
        data = {
...
export default async function handler(req, res) {
    ...
    const { token, data, pointId, immobileId } = JSON.parse(body)
    let resFinal = { status: "ERROR", error: {}, id: 0, message: "" }
    switch (method) {
        case "POST":
            let point: ImmobilePoint = data
            if (token === "tokenbemseguro") {
                const resAdd = await handleAddImmobilePoint(point).then(res => res)
                if (resAdd === 0) {
