...
export default function ImmobilePointPage(props: ImmobilePointPageProps) {
    ...
    const handleDeleteClick = async (immobilePoint, index) => {
        ...
        const res = await fetch("api/immobilePoint", {
            method: "DELETE",
    ...
    const handleEditClick = async (immobilePoint, index?) => {
        handleSetIsLoading(true)
        setIsForShow(false)
        let localImmobilePoint: ImmobilePoint = await fetch("api/immobilePoint/" + immobilePoint?.id).then((res) => res.json()).then((res) => res.data)
        localImmobilePoint = {
            ...localImmobilePoint,
        }
        handleSetIsLoading(false)
